...
directive('uploadForm', function ($rootScope) {
	...
	return {
		...
		template: '' +
			'<p ng-bind="file | limitTo: 50" style="white-space: nowrap"></p><div ng-if="percent" ng-style="{\'width\': width + \'px\'}" style="height: 100%; background-color: gray; float: left"><div ng-class="{\'uploading\': bartype == \'uploading\', \'processing\': bartype == \'processing\'}" ng-style="{\'width\': percent + \'%\'}" style="height: 100%; float: left">&nbsp</div></div><div ng-if="percent" style="float: right; width: 50px; text-align: right">{{percent}}%</div><div ng-if="!percent && timestamp">Seconds processed: {{timestamp}}</div>',
